...
class Input {
	...
	public static function force_release(k:Key):Void {
		...
		if (keyheld[keycode] >= 0) {
			...
			last[keycode] = Keystate.force_release;
			keyheld[keycode] = -1;
		}
	}
	...
	// delay_pressed(): returns true at specific frame counts after the key's been pressed.
	...
	private static function update() {
		...
		#if cpp
		// Hack to get around Event.PASTE not registering on cpp
		if (paste) {
			paste = false;
			handlepaste(null);
		}
	...
	private static function handlekeydown(event:KeyboardEvent) {
		...
		if (lookup.exists(keycode) && current[keycode] != Keystate.force_release) {
			if (iskeycodeheld(current[keycode])) {
